...
## Description
This is a simple demo which builds on top of the [Reaction-Diffusion tutorial](https://github.com/axyorah/reaction_diffusion_tutorial) and shows what happens if patterns discussed in the tutorial are transfered onto some interesting geometrical shapes, like snail shells.
...
It is convenient to interpret `xA` as "prey" and `xB` as "predator". The equations tell us that whenever a "unit" of prey encounters two "units" of predators it doesn't end well for the prey (their concentration is decreased  with a rate proportional to  at a given point in space; last term in `xA` equation) but it ends quite well for the predators (predator's "concentration" is increased with the same rate; second term in `xB` equation). Additionally, prey constantly grows with the growth rate proportional to some coefficient `f`. Growth, however, slows down as the "concentration" of prey get closer to a threshold of `1` (second term in `xA` equation). At the same time predator constantly dies off with a net decay rate proportional to `k-f` (last term in `xB` equation). Here `k` can be interpreted as true predator decay coefficient. It's interesting to note that if `f > k` predator will never die, and its growth will be modulated by local availability of prey.
...
```
On Windows you can run the same command, but instead of `$(pwd)` manually type in the absolute path to this directory (with brackets). If it still gives you troubles try using git bash or any other bash instead of default Windows `cmd`. Alternatively, just run the above command without `-v $(pwd):/shells` - this parameter is only needed to share the files between your machine and the container.
...
To run the demo locally you need to have [node.js](https://nodejs.org/en/) installed. Once you have it installed copy this repo to your machine:
```
$ git clone https://github.com/axyorah/snailshells.git
```

Set up the environment. From the directory where you've copied this repo to run:
```
...
Demo will available at `localhost:8181` in your browser.

### Adding your own custom textures
If you're running the demo locally or if you've passed `-v $(pwd):/shells` parameter when starting the docker container, you can easily add your own custom textures to demo. To do it:
